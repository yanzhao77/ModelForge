...
当前 `todo.md` 中的 P0 模型就绪状态与首次使用向导、P1 智能体创建/远程模型可靠性及 P2 验证和跨平台打包准备均已完成。下一优先级是**真实环境验证与受控 Beta 发布**：在原生 Windows/Linux 上构建并安装验证、配置 NVIDIA Runner 完成 GPU Smoke、在预置本地模型的 CPU Runner 上进行真实推理 smoke；以上条件完成并经用户确认后，再创建 `v0.1.1-beta.1` GitHub Pre-release。[4]

`0.1.2` 已完成首个可合并切片：Agent 定义保存不含密钥的本地/远程模型目标路由并在执行时由后端重新校验已验证提供商；创建向导提供可克隆模板、可编辑提示与最小权限/审批预览；远程服务已覆盖认证、限流、服务错误、Responses→Chat Completions 回退和流式中断的安全诊断。后续产品迭代应基于真实使用反馈复核首次使用向导的 7 天抑制、默认模型切换和三语言文本完整性。所有动作仍须保持用户显式确认原则，特别是不得由向导或模板自动创建 Agent Run、下载模型或启动训练。
