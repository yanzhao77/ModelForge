...
def application_stylesheet(p: dict) -> str:
    return f"""
    * {{ font-family: {t.FONT_UI}; color: {p["text"]}; letter-spacing: 0px; }}
    QMainWindow, QDialog, QWidget#AppShell {{ background: {p["bg"]}; }}
    QFrame#TopBar {{ background: {p["surface"]}; border-bottom: 1px solid {p["border"]}; }}
    QFrame#SideRail {{ background: {p["sidebar"]}; border-right: 1px solid {p["border"]}; }}
    ...
    QFrame[toolbar="true"] {{ background: transparent; border: 0; }}
    QFrame[settingsRow="true"] {{ background: transparent; border-bottom: 1px solid {p["border"]}; }}
    ...
    QLabel[role="eyebrow"] {{ color: {p["muted"]}; font-size: 11px; font-weight: 600; letter-spacing: 0px; }}
    QLabel[role="pageTitle"] {{ color: {p["text"]}; font-size: 22px; font-weight: 600; }}
    QLabel[role="sectionTitle"] {{ color: {p["text"]}; font-size: 15px; font-weight: 600; }}
    ...
    QPushButton {{ background: {p["surface"]}; border: 1px solid {p["border_strong"]}; border-radius: {t.RADIUS_SM}px; padding: 6px 12px; min-height: 20px; font-size: 13px; font-weight: 500; }}
    QPushButton:hover {{ background: {p["hover"]}; }} QPushButton:pressed {{ background: {p["selection"]}; }}
    QPushButton:focus {{ border: 2px solid {p["focus"]}; padding: 5px 11px; }}
    ...
    QPushButton[nav="true"] {{ text-align: left; color: {p["muted"]}; background: transparent; border: 1px solid transparent; padding: 6px 10px; min-height: 18px; font-size: 13px; font-weight: 500; }}
    ...
    QPushButton[nav="true"]:focus {{ border: 2px solid {p["focus"]}; padding: 5px 9px; }}
    QPushButton[iconOnly="true"], QToolButton {{ min-width: 30px; min-height: 30px; padding: 0px; border-radius: {t.RADIUS_SM}px; }}
    ...
    QLineEdit:focus, QTextEdit:focus, QTextBrowser:focus, QPlainTextEdit:focus, QComboBox:focus, QSpinBox:focus {{ border: 2px solid {p["focus"]}; padding: 7px 9px; }}
    QCheckBox:focus, QTabBar::tab:focus {{ outline: 2px solid {p["focus"]}; outline-offset: 2px; }}
    ...
    QTabBar::tab {{ background: transparent; color: {p["muted"]}; padding: 7px 14px; border: 1px solid transparent; border-radius: {t.RADIUS_SM}px; font-weight: 500; }}
    QTabBar::tab:selected {{ color: {p["text"]}; background: {p["selection"]}; border-color: {p["border"]}; font-weight: 600; }}
    ...
    QTableWidget, QListWidget, QTreeWidget {{ background: {p["surface"]}; alternate-background-color: {p["surface_subtle"]}; border: 1px solid {p["border"]}; border-radius: {t.RADIUS_SM}px; outline: 0; gridline-color: {p["border"]}; }}
    QTableWidget:focus, QListWidget:focus, QTreeWidget:focus {{ border: 2px solid {p["focus"]}; }}
