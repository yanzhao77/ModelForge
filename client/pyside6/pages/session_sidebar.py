...
from i18n.ui_localizer import current, format_api_error, format_text, localize_tree, text
...
class SessionSidebar(QWidget, AsyncApiMixin):
    ...
    def retranslate(self, translator=None) -> None:
        ...
        self.title.setText(text("对话列表", locale))
        self.new_btn.setText(text("+ 新建对话", locale))
    ...
    def refresh(self):
        ...
        self.status.setText(format_text("正在同步会话…"))
    ...
    def _render_sessions(self, sessions):
        ...
        for session in sessions:
            item = QListWidgetItem(format_text("{title}\n{count} 条消息", title=session["title"], count=session.get("message_count", 0)))
            item.setData(Qt.UserRole, session["id"])
            self.session_list.addItem(item)
            if session["id"] == self.current_session_id:
                self.session_list.setCurrentItem(item)
        self._empty_toggle(not sessions)
        self.status.setText(format_text("已同步 {count} 个会话。", count=len(sessions)))

    def _load_failed(self, error):
        self._set_loading(False)
        self.status.setText(format_text("会话同步失败：{error}", error=format_api_error(error)))

    def create_new_session(self):
        if self._loading:
            return
        self._set_loading(True)
        self.status.setText(format_text("正在创建新对话…"))
    ...
    def _show_menu(self, pos):
        ...
        session_id = item.data(Qt.UserRole)
        menu = QMenu(self)
        rename = menu.addAction(format_text("重命名"))
        clear = menu.addAction(format_text("清空消息"))
        menu.addSeparator()
        delete = menu.addAction(format_text("删除"))
    ...
    def _rename(self, session_id):
        title, ok = QInputDialog.getText(self, format_text("重命名"), format_text("新的标题:"))
    ...
    def _clear(self, session_id):
        if QMessageBox.question(self, format_text("确认"), format_text("确定清空此对话的所有消息？")) != QMessageBox.Yes:
            return
        self._set_loading(True)
    ...
    def _delete(self, session_id):
        if QMessageBox.question(self, format_text("确认"), format_text("确定删除此对话？")) != QMessageBox.Yes:
            return
        self._set_loading(True)
    ...
    def _action_failed(self, error):
        self._set_loading(False)
        self.status.setText(format_text("会话操作失败：{error}", error=format_api_error(error)))
        QMessageBox.warning(self, format_text("会话操作失败"), format_api_error(error))
